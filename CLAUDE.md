...
**Multiple fichas per patient / per weekday**: a patient can have more than one active plan at once — activating one no longer deactivates the others (`ativar`/`desativar`). Each `nutri_planos` row carries `dias_semana` (JSON array of `Carbon::dayOfWeek` ints, 0=Sun…6=Sat; empty/null = every day), edited via the weekday pills in the plan editor header. `Paciente::planosAtivos()` returns all active fichas; `Paciente::escolherPlanoDoDia($ativos, $dia)` picks the day's ficha (a day-specific one wins over an "every day" one), and `planoDoDia()`/`planoAtivo()` (today's) wrap it. The patient portal (`PortalController@plano`, `?dia=`) shows a weekday tab bar and renders that day's ficha; the shopping list aggregates all active fichas. Day labels come from `PlanoAlimentar::diasSemanaLabels()`.
