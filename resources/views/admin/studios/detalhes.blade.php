...
<head>
    ...
    <title>Detalhes - {{ $studio->nome ?? 'Studio' }}</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-delete { background: transparent; border: 1px solid var(--text-muted); color: var(--text-muted); margin-left: auto; }
...
<div class="container">
    ...
    <div class="header-card">
        ...
        <div class="header-info">
            <h1>{{ $studio->nome ?? 'N/A' }}</h1>
            <p>{{ $studio->modalidades ?? 'Modalidades não informadas' }}</p>
    ...
    {{-- DADOS DO STUDIO --}}
    <h2 class="section-title">Dados Operacionais</h2>
    <div class="info-section">
        <div class="info-row">
            <div>
                <p class="info-label">Valor da Aula Avulsa</p>
                <p class="info-value" style="color: var(--primary); font-weight: 700;">
                    R$ {{ number_format($studio->valor_aula ?? 0, 2, ',', '.') }}
                </p>
            </div>
            <div>
                <p class="info-label">Capacidade por Horário</p>
                <p class="info-value">{{ $studio->capacidade_padrao ?? 10 }} alunos</p>
            </div>
        </div>
        <div class="info-row" style="margin-top: 16px;">
            <div>
                <p class="info-label">Faturamento (Mês)</p>
                <p class="info-value" style="color: #00ff88; font-size: 1.3rem; font-weight: 900;">
                    R$ {{ number_format($faturamentoMes ?? 0, 2, ',', '.') }}
                </p>
            </div>
            <div>
                <p class="info-label">Descrição</p>
                <p class="info-value">{{ $studio->descricao ?? 'Sem descrição' }}</p>
            </div>
        </div>
    </div>
    ...
    @else
        ...
        @if($studio->asaas_wallet_id)
            <div style="background: rgba(212, 255, 0, 0.07); padding: 16px 20px; border-radius: 12px; border: 1px solid rgba(212, 255, 0, 0.3); margin-bottom: 16px; display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    ...
                    <p style="margin: 4px 0 0; color: #a0a0a0; font-size: 0.72rem; font-family: monospace;">Wallet ID: {{ $studio->asaas_wallet_id }}</p>
                </div>
            </div>
        @else
            <div style="background: rgba(255, 165, 0, 0.08); padding: 16px 20px; border-radius: 12px; border: 1px solid rgba(255, 165, 0, 0.3); margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap;">
                ...
                <form method="POST" action="{{ route('admin.studios.criar-asaas', $studio->id) }}"
        ...
        <div class="action-buttons" style="margin-top: 8px;">
            <form method="POST" action="{{ route('admin.studios.deletar', $studio->id) }}"
