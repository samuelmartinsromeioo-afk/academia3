...
<head>
    ...
    <title>Studios - Administração</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-select:focus,
        .filter-input:focus { border-color: var(--primary); background: rgba(255, 255, 255, 0.06); }
        ...
        @media (max-width: 768px) {
            ...
            .studio-name { flex-direction: column; gap: 4px; }
        }
    </style>
</head>
<body class="ed-page">
...
<div class="container">
    ...
    {{-- FILTROS --}}
    <div class="filters">
        <form method="GET" action="{{ route('admin.studios.lista') }}" style="display: flex; gap: 16px; flex-wrap: wrap; width: 100%;">
                    ...
                    @foreach($studios as $studio)
                        <tr>
                            <td>
                                <div class="studio-name">
                                    <div class="studio-avatar">
                                        {{ substr($studio->nome ?? 'S', 0, 1) }}
                                    </div>
                                    <span>{{ $studio->nome ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td style="color: var(--text-muted);">{{ $studio->email ?? 'N/A' }}</td>
                            <td style="color: var(--text-muted);">{{ $studio->cnpj ?? 'N/A' }}</td>
                            <td style="color: var(--text-muted);">{{ $studio->cidade }}/{{ $studio->estado }}</td>
                            <td>
                            ...
                            <td style="color: var(--text-muted);">
                                {{ $studio->created_at ? $studio->created_at->format('d/m/Y') : 'N/A' }}
                            </td>
                            <td>
