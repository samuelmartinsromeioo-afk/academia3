...
<head>
    ...
    <title>Academias - Administração</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-back { background: transparent; border: 1px solid var(--primary); color: var(--primary); padding: 10px 16px; border-radius: 8px; cursor: pointer; font-weight: 700; font-size: 0.8rem; transition: 0.2s; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; }
        ...
        .filter-select, .filter-input { background: var(--input-bg); border: 1px solid var(--border); color: var(--text-main); padding: 10px 14px; border-radius: 8px; font-family: inherit; font-size: 0.9rem; outline: none; transition: 0.2s; }
        ...
        .btn-view { background: transparent; border: 1px solid var(--primary); color: var(--primary); padding: 6px 12px; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.7rem; text-transform: uppercase; transition: 0.2s; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; }
        ...
        @media (max-width: 768px) { .filters { flex-direction: column; } .table-container { overflow-x: auto; } th, td { padding: 12px 8px; font-size: 0.7rem; } .acad-name { flex-direction: column; gap: 4px; } }
    </style>
</head>
<body class="ed-page">

<div class="top-bar">
    <h2><i class="ph ph-buildings"></i> Gerenciar Academias</h2>
...
<div class="container">
    ...
    <div class="filters">
        <form method="GET" action="{{ route('admin.academias.lista') }}" style="display: flex; gap: 16px; flex-wrap: wrap; width: 100%;">
            <div class="filter-group">
                <label>Status:</label>
                <select name="status" class="filter-select" onchange="this.form.submit()">
                    <option value="todos" {{ $filtro === 'todos' ? 'selected' : '' }}>Todos</option>
                    <option value="pendente" {{ $filtro === 'pendente' ? 'selected' : '' }}>Pendentes</option>
                    <option value="aprovado" {{ $filtro === 'aprovado' ? 'selected' : '' }}>Aprovadas</option>
                    <option value="rejeitado" {{ $filtro === 'rejeitado' ? 'selected' : '' }}>Rejeitadas</option>
                </select>
            </div>
            <div class="filter-group">
                <label>Buscar:</label>
    ...
    @if($academias && count($academias) > 0)
                ...
                <tbody>
                    @foreach($academias as $academia)
                        <tr>
                            <td>
                                <div class="acad-name">
                                    <div class="acad-avatar">{{ substr($academia->nome ?? 'A', 0, 1) }}</div>
                                    <span>{{ $academia->nome ?? 'N/A' }}</span>
                                </div>
                            </td>
                            <td style="color: var(--text-muted);">{{ $academia->email ?? 'N/A' }}</td>
                            <td style="color: var(--text-muted);">{{ $academia->cnpj ?? 'N/A' }}</td>
                            <td style="color: var(--text-muted);">{{ $academia->cidade }}/{{ $academia->estado }}</td>
                            <td style="color: var(--text-muted);">{{ $academia->quantidade_alunos ?? '—' }}</td>
                            <td>
                                <span class="status-badge status-{{ $academia->status ?? 'pendente' }}">
                                    {{ ucfirst($academia->status ?? 'pendente') }}
                                </span>
                            </td>
                            <td style="color: var(--text-muted);">{{ $academia->created_at ? $academia->created_at->format('d/m/Y') : 'N/A' }}</td>
                            <td>
                                <a href="{{ route('admin.academias.detalhes', $academia->id) }}" class="btn-view"><i class="ph ph-eye"></i> Ver</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="table-container">
            <div class="empty-state">
                ...
                <p>Nenhuma academia encontrada com esses critérios.</p>
            </div>
        </div>
    @endif
</div>
