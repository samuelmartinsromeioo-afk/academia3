    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .top-bar {
            ...
            padding: 18px 40px; background: rgba(0, 0, 0, 0.3);
        ...
        .welcome h1 em { color: var(--loja-color); font-style: normal; }
        ...
        .search-wrapper i { color: var(--loja-color); }
        .search-wrapper input {
            flex: 1; background: transparent; border: none; padding: 13px 12px;
            color: #fff; outline: none; font-size: 0.9rem; font-family: inherit;
        }

        .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 22px; }

        .card { background: var(--card-bg); border: 1px solid var(--border); border-radius: 18px; overflow: hidden; transition: 0.25s; display: flex; flex-direction: column; }
        ...
        .card-badge {
            ...
            padding: 5px 11px; border-radius: 20px; border: 1px solid rgba(255,157,46,0.4);
        }
        ...
        .prod-count { font-weight: 800; font-size: 0.85rem; color: var(--text-muted); }
        .prod-count strong { color: var(--text-main); }

        .btn-detalhes {
            background: var(--primary); color: #000; border: none; border-radius: 10px; padding: 10px 16px;
...
<script>
    const inputBusca = document.getElementById('buscaLoja');
    if (inputBusca) {
        inputBusca.addEventListener('input', () => {
            const termo = inputBusca.value.toLowerCase().trim();
            let visiveis = 0;
            document.querySelectorAll('#gridLojas .card').forEach(card => {
