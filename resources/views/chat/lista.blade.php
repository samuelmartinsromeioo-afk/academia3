...
<body>
    <div class="top-bar">
        ...
        <span class="title"><i class="fas fa-comments"></i> Mensagens</span>
    </div>
    ...
    <div class="container">
        ...
        @if($contatos->isEmpty())
            <div class="empty"><i class="fas fa-comment-slash"></i><p>{{ $eu === 'personal' ? 'Você ainda não tem alunos para conversar.' : 'Você ainda não tem um personal para conversar.' }}</p></div>
        @else
            @foreach($contatos as $c)
                <a href="{{ route('chat.conversa', $c->id) }}" class="conv">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode($c->nome) }}&background=F4BE16&color=000" alt="">
                    <div class="info">
