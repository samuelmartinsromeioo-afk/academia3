...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .alert-success { background: rgba(0,230,118,0.12); color: var(--green); border: 1px solid var(--green); }
        ...
        .btn-pular { display: block; text-align: center; margin-top: 14px; color: var(--text-muted); font-size: 0.82rem; font-weight: 700; }
        .btn-pular:hover { color: var(--primary); }
...
<body class="ed-page">
    <div class="top-bar">
        ...
        <span class="title"><i class="ph ph-first-aid"></i> Anamnese — {{ $cliente->nome }}</span>
    </div>

    <div class="container">
        <div class="ed-eyebrow"><i class="ph ph-first-aid"></i> Avaliação</div>
        <h1 class="ed-h"><span class="ed-mark">Anamnese</span></h1>
        <p class="subtitle">Preencha a anamnese de <strong>{{ $cliente->nome }}</strong>. Essas informações ajudam a montar um treino seguro e eficiente.</p>
        ...
        <form method="POST" action="{{ route('academia.alunos.anamnese.salvar', $cliente->id) }}">
            @csrf
            ...
            <div class="panel">
                ...
                <textarea name="observacoes" placeholder="Algo mais que o personal/professor deva saber?">{{ old('observacoes', $anamnese->observacoes) }}</textarea>
            </div>

            <button type="submit" class="btn-salvar"><i class="ph ph-floppy-disk"></i> Salvar anamnese</button>
        </form>

        <a href="{{ route('academia.alunos') }}" class="btn-pular"><i class="ph ph-clock-counter-clockwise"></i> Pular por enquanto — preencher depois</a>
    </div>
</body>
