...
<body>
    ...
    <div class="container">
        ...
        <header style="margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="display: flex; gap: 10px;">
                <button id="btnOpenCancelarDia" class="btn-save" style="width: auto; padding: 12px 20px; margin: 0; font-size: 0.75rem; background: rgba(255,68,68,0.15); color: var(--error); border: 1px solid rgba(255,68,68,0.3);">
                    <i class="fas fa-calendar-times"></i> CANCELAR DIA
                </button>
                <button id="btnOpenBloquearFixo" class="btn-save" style="width: auto; padding: 12px 20px; margin: 0; font-size: 0.75rem; background: rgba(255,165,0,0.15); color: #ffa500; border: 1px solid rgba(255,165,0,0.3);">
                    <i class="fas fa-lock"></i> BLOQUEAR FIXO
                </button>
    ...
    {{-- MODAL CANCELAR DIA INTEIRO --}}
    <div id="modalCancelarDia" class="modal-overlay">
        <div class="modal-content" style="max-width: 460px;">
            <h2 style="color: var(--error); font-size: 1.4rem; margin-top: 0; font-weight: 900;">
                <i class="fas fa-calendar-times"></i> CANCELAR DIA INTEIRO
            </h2>
            <p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 20px;">
                Todos os horários do dia selecionado serão cancelados (respeitando a regra de 24h de antecedência).
            </p>
            <form action="{{ route('personal.cancelarDia') }}" method="POST">
                @csrf
                <label>Selecione o Dia</label>
                <div class="input-wrapper">
                    <i class="fas fa-calendar" style="color: var(--error);"></i>
                    <input type="date" name="data" id="cancelarDia_data" required
                           min="{{ now()->addDay()->format('Y-m-d') }}">
                </div>
                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button type="button" onclick="closeModalCancelarDia()" class="btn-save btn-cancel">Voltar</button>
                    <button type="submit" class="btn-save" style="background: var(--error); color: #fff;">
                        <i class="fas fa-trash-alt"></i> Confirmar Cancelamento
                    </button>
                </div>
            </form>
        </div>
    ...
    {{-- MODAL BLOQUEAR HORÁRIO FIXO --}}
    <div id="modalBloquearFixo" class="modal-overlay">
        <div class="modal-content" style="max-width: 460px;">
            <h2 style="color: #ffa500; font-size: 1.4rem; margin-top: 0; font-weight: 900;">
                <i class="fas fa-lock"></i> BLOQUEAR HORÁRIO FIXO
            </h2>
            <p style="color: var(--text-muted); font-size: 0.85rem; margin-bottom: 20px;">
                Bloqueia o mesmo intervalo de horário em todos os dias pelos próximos X dias.
            </p>
            <form action="{{ route('personal.bloquearFixo') }}" method="POST">
                @csrf
                @if($errors->has('horario'))
                    <p style="color: var(--error); font-size: 0.8rem; margin-bottom: 10px;">
                        <i class="fas fa-exclamation-triangle"></i> {{ $errors->first('horario') }}
                    </p>
                @endif
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                ...
                <label style="margin-top: 15px;">Repetir pelos próximos (dias)</label>
                <div class="input-wrapper">
                    <i class="fas fa-redo" style="color: #ffa500;"></i>
                    <input type="number" name="dias" min="1" max="90" value="30" required>
                </div>
                <label style="margin-top: 15px;">Motivo do Bloqueio</label>
                <div class="input-wrapper">
                    <i class="fas fa-pen"></i>
                    <input type="text" name="descricao" placeholder="Ex: Almoço / Aula de idiomas / Descanso">
                </div>
                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button type="button" onclick="closeModalBloquearFixo()" class="btn-save btn-cancel">Voltar</button>
                    <button type="submit" class="btn-save" style="background: #ffa500; color: #000;">
                        <i class="fas fa-lock"></i> Aplicar Bloqueio Fixo
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        const modalCancelarDia   = document.getElementById('modalCancelarDia');
        const modalBloquearFixo  = document.getElementById('modalBloquearFixo');

        document.getElementById('btnOpenCancelarDia').onclick = () => {
            document.getElementById('cancelarDia_data').value = '';
            modalCancelarDia.style.display = 'block';
        };

        document.getElementById('btnOpenBloquearFixo').onclick = () => {
            modalBloquearFixo.style.display = 'block';
        };

        function closeModalCancelarDia()  { modalCancelarDia.style.display  = 'none'; }
        function closeModalBloquearFixo() { modalBloquearFixo.style.display = 'none'; }
        ...
        window.onclick = (e) => {
            ...
            if (e.target == modalCancelarDia) closeModalCancelarDia();
            if (e.target == modalBloquearFixo) closeModalBloquearFixo();
        }
