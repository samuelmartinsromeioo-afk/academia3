...
<div class="top-bar">
    <div style="display:flex; align-items:center; gap:12px;">
        <a href="{{ route('academia.avaliacao-fisica') }}" class="btn-back"><i class="ph ph-arrow-left"></i> Voltar</a>
    </div>
    <div style="display:flex; align-items:center; gap:12px;">
        <img src="{{ $academia->foto ? asset('storage/'.$academia->foto) : 'https://cdn-icons-png.flaticon.com/512/3135/3135715.png' }}" style="width:38px; height:38px; border-radius:50%; border:2px solid var(--primary); object-fit:cover;">
...
<div class="container">
    ...
    <div class="tipo-tabs">
        <a href="{{ route('academia.avaliacao-fisica.aluno', $cliente->id) }}{{ $mesFiltro ? '?mes='.$mesFiltro : '' }}" class="tipo-tab {{ !$tipoFiltro ? 'active' : '' }}">
            <i class="ph ph-list"></i> Todos
        </a>
        @foreach($tiposCriaveis as $key)
        <a href="{{ route('academia.avaliacao-fisica.aluno', $cliente->id) }}?tipo={{ $key }}{{ $mesFiltro ? '&mes='.$mesFiltro : '' }}" class="tipo-tab {{ $tipoFiltro === $key ? 'active' : '' }}">
            <i class="ph {{ $tipos[$key]['icon'] }}"></i> {{ $tipos[$key]['label'] }}
        </a>
        @endforeach
        <a href="{{ route('academia.avaliacao-fisica.aluno', $cliente->id) }}?tipo=resumo{{ $mesFiltro ? '&mes='.$mesFiltro : '' }}" class="tipo-tab {{ $tipoFiltro === 'resumo' ? 'active' : '' }}">
    ...
    @else
        ...
        <div class="card">
            <div class="registro-head">
                ...
                <form action="{{ route('academia.avaliacao-fisica.destroy', $r->id) }}" method="POST" onsubmit="return confirm('Excluir este registro?')">
                    @csrf
                    @method('DELETE')
...
<div id="modalRegistro" class="modal-overlay">
    <div class="modal-content">
        ...
        <form action="{{ route('academia.avaliacao-fisica.store', $cliente->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-row">
...
<script>
    ...
    function filtrarMes(mes) {
        ...
        window.location.href = '{{ route('academia.avaliacao-fisica.aluno', $cliente->id) }}' + (qs ? '?' + qs : '');
    }
