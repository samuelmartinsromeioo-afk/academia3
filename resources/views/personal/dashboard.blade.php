...
<head>
    ...
    <style>
        ...
        .dropdown-menu { display: none; position: absolute; top: 50px; left: 0; background: var(--card-bg); border: 1px solid var(--border); border-radius: 16px; width: 260px; z-index: 1000; overflow: hidden; box-shadow: 0 15px 35px rgba(0,0,0,0.6); }
        .menu-rating-header { padding: 20px; background: rgba(212, 255, 0, 0.05); border-bottom: 1px solid var(--border); text-align: center; }
        ...
        .dropdown-menu button { display: flex; align-items: center; gap: 12px; padding: 15px 20px; color: #fff; text-decoration: none; font-size: 14px; width: 100%; text-align: left; background: none; border: none; cursor: pointer; transition: 0.2s; }
        .dropdown-menu button:hover { background: rgba(255,255,255,0.05); color: var(--primary); }

        .profile-header { display: flex; align-items: center; gap: 20px; }
        .avatar-img { width: 65px; height: 65px; border-radius: 50%; border: 3px solid var(--primary); object-fit: cover; background: #222; }

        .container { max-width: 1200px; margin: 30px auto; padding: 0 20px; }
        
        /* Navegação de Semana */
        .calendar-nav { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; }
        .nav-link { color: var(--text-main); text-decoration: none; background: var(--card-bg); padding: 8px 15px; border-radius: 10px; border: 1px solid var(--border); font-size: 0.8rem; transition: 0.3s; }
        .nav-link:hover { border-color: var(--primary); color: var(--primary); }

        .agenda-section { display: grid; grid-template-columns: 2fr 1fr; gap: 20px; margin-top: 20px; }
        .calendar-card { background: var(--card-bg); border-radius: 24px; padding: 30px; border: 1px solid var(--border); }
        .week-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 10px; text-align: center; }
        .day-col { background: rgba(255,255,255,0.02); padding: 15px 5px; border-radius: 12px; border: 1px solid var(--border); min-height: 100px; cursor: pointer; transition: 0.3s; display: flex; flex-direction: column; align-items: center; justify-content: center; position: relative; }
        .day-col:hover { background: rgba(212, 255, 0, 0.05); border-color: rgba(212, 255, 0, 0.3); }
        .day-col.active { background: rgba(212, 255, 0, 1); border-color: var(--primary); }
        .has-event::after { content: ''; width: 6px; height: 6px; background: var(--primary); border-radius: 50%; position: absolute; bottom: 12px; box-shadow: 0 0 10px var(--primary); }

        .agenda-notes { background: linear-gradient(135deg, #16181d 0%, #0a0b0d 100%); padding: 25px; border-radius: 24px; border: 1px solid var(--border); min-height: 400px; }
        .summary-item { background: rgba(255, 255, 255, 0.03); border-radius: 12px; padding: 15px; margin-top: 12px; border-left: 4px solid var(--primary); display: flex; justify-content: space-between; align-items: center; }

        /* Botão Cancelar Agenda */
        .btn-delete-agenda { background: rgba(255, 68, 68, 0.1); color: var(--error); border: 1px solid rgba(255, 68, 68, 0.2); width: 35px; height: 35px; border-radius: 8px; cursor: pointer; transition: 0.3s; }
        ...
        /* Modais */
        .modal-overlay { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.92); backdrop-filter: blur(8px); z-index: 2000; overflow-y: auto; padding: 40px 0; }
        .modal-content { background: var(--card-bg); max-width: 750px; margin: auto; padding: 35px; border-radius: 28px; border: 1px solid var(--border); }
        
        .finance-card { background: rgba(212, 255, 0, 0.05); border: 1px solid var(--primary); border-radius: 16px; padding: 20px; text-align: center; margin-bottom: 20px; }
        .finance-value { font-size: 2rem; font-weight: 900; color: var(--primary); display: block; }

        .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }
        .full-width { grid-column: span 2; }
        .input-wrapper { display: flex; align-items: center; background: rgba(255,255,255,0.04); border: 1px solid var(--border); border-radius: 10px; padding: 0 12px; margin-top: 5px; }
        .input-wrapper input { flex: 1; background: transparent; border: none; padding: 12px; color: #fff; outline: none; font-size: 0.9rem; }
        .input-wrapper i { color: var(--primary); width: 20px; text-align: center; }
        label { font-size: 0.65rem; color: var(--text-muted); text-transform: uppercase; font-weight: 800; margin-top: 12px; display: block; letter-spacing: 0.5px; }

        .btn-save { background: var(--primary); color: #000; width: 100%; padding: 15px; border-radius: 12px; font-weight: 900; border: none; cursor: pointer; text-transform: uppercase; transition: 0.3s; margin-top: 20px; }
...
<body>
    ...
    <div class="top-bar">
        <div class="menu-container">
            ...
            <div class="dropdown-menu" id="dropdownMenu">
                ...
                <form action="{{ route('logout') }}" method="POST"> @csrf <button type="submit" style="color: var(--error)"><i class="fas fa-power-off"></i> Sair</button></form>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <form action="{{ route('personal.storeHorario') }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div><label>Início</label><div class="input-wrapper"><i class="fas fa-clock"></i><input type="time" name="hora_inicio" required></div></div>
                    <div><label>Fim</label><div class="input-wrapper"><i class="fas fa-clock"></i><input type="time" name="hora_fim" required></div></div>
                </div>
                <label>Motivo</label>
    ...
    <script>
        ...
        // NOVO EVENTO PARA O BOTÃO ALUNOS
        document.getElementById('btnOpenAlunos').onclick = () => { 
            window.location.href = "{{ route('personal.alunos') }}"; 
        };
        ...
        window.onload = () => {
            const today = document.querySelector('.day-col.today');
